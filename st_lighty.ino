...
void setup() {
    ...
    // Initialize LEDs
    FastLED.addLeds<WS2812B, STRIP_1>(leds[0], NUM_LEDS);
    FastLED.addLeds<WS2812B, STRIP_2>(leds[1], NUM_LEDS);
    FastLED.addLeds<WS2812B, STRIP_3>(leds[2], NUM_LEDS);
    FastLED.addLeds<WS2812B, STRIP_4>(leds[3], NUM_LEDS);
    FastLED.addLeds<WS2812B, STRIP_5>(leds[4], NUM_LEDS);
    FastLED.addLeds<WS2812B, STRIP_6>(leds[5], NUM_LEDS);
    FastLED.addLeds<WS2812B, STRIP_7>(leds[6], NUM_LEDS);
}
...
void Graph_Frequencies() {
    for (int freq_band = 0; freq_band < 7; freq_band++) {
        int lit_leds = floor((Frequencies_Left[freq_band] + Frequencies_Right[freq_band] / 2046) * NUM_LEDS);

        for (int cur_led = 0; cur_led < lit_leds; cur_led++) {
            leds[freq_band][cur_led] = CRGB::SpringGreen;
        }

        fadeToBlackBy(leds[freq_band], NUM_LEDS, 10);
